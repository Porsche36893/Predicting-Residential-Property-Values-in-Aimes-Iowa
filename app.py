...
with c2: 
    ...
    input.loc[0, "Street"] = st.text_input("Street: Type of road access")
    input.loc[0, "LotShape"] = st.text_input("LotShape: General shape of property")
    input.loc[0, "LandContour"] = st.text_input("LandContour: Flatness of the property")
    input.loc[0, "Utilities"] = st.text_input("Utilities: Type of utilities available")
    input.loc[0, "LotConfig"] = st.text_input("LotConfig: Lot configuration")
    input.loc[0, "LandSlope"] = st.text_input("LandSlope: Slope of property")
    input.loc[0, "Neighborhood"] = st.text_input("Neighborhood: Physical locations within Ames city limits")
    input.loc[0, "Condition1"] = st.text_input("Condition1: Proximity to main road or railroad")
    input.loc[0, "Condition2"] = st.text_input("Condition2: Proximity to main road or railroad (if a second is present)")
    input.loc[0, "BldgType"] = st.text_input("BldgType: Type of dwelling")
    input.loc[0, "HouseStyle"] = st.text_input("HouseStyle: Style of dwelling")
    input.loc[0, "RoofStyle"] = st.text_input("RoofStyle: Type of roof")
    input.loc[0, "RoofMatl"] = st.text_input("RoofMatl: Roof material")
    input.loc[0, "Exterior1st"] = st.text_input("Exterior1st: Exterior covering on house")
    input.loc[0, "Exterior2nd"] = st.text_input("Exterior2nd: Exterior covering on house (if more than one material)")
    input.loc[0, "MasVnrType"] = st.text_input("MasVnrType: Masonry veneer type")
    input.loc[0, "ExterQual"] = st.text_input("ExterQual: Exterior material quality")
    input.loc[0, "ExterCond"] = st.text_input("ExterCond: Present condition of the material on the exterior")
    input.loc[0, "Foundation"] = st.text_input("Foundation: Type of foundation")
    input.loc[0, "BsmtQual"] = st.text_input("BsmtQual: Height of the basement")
    input.loc[0, "BsmtCond"] = st.text_input("BsmtCond: General condition of the basement")
    input.loc[0, "BsmtExposure"] = st.text_input("BsmtExposure: Walkout or garden level basement walls")
    input.loc[0, "BsmtFinType1"] = st.text_input("BsmtFinType1: Quality of basement finished area")
    input.loc[0, "BsmtFinType2"] = st.text_input("BsmtFinType2: Quality of second finished area (if present)")
    input.loc[0, "Heating"] = st.text_input("Heating: Type of heating")
    input.loc[0, "HeatingQC"] = st.text_input("HeatingQC: Heating quality and condition")
    input.loc[0, "CentralAir"] = st.text_input("CentralAir: Central air conditioning")
    input.loc[0, "Electrical"] = st.text_input("Electrical: Electrical system")
    input.loc[0, "KitchenQual"] = st.text_input("KitchenQual: Kitchen quality")
    input.loc[0, "Functional"] = st.text_input("Functional: Home functionality rating")
    input.loc[0, "FireplaceQu"] = st.text_input("FireplaceQu: Fireplace quality")
    input.loc[0, "GarageType"] = st.text_input("GarageType: Garage location")
    input.loc[0, "GarageFinish"] = st.text_input("GarageFinish: Interior finish of the garage")
    input.loc[0, "GarageQual"] = st.text_input("GarageQual: Garage quality")
    input.loc[0, "GarageCond"] = st.text_input("GarageCond: Garage condition")
    input.loc[0, "PavedDrive"] = st.text_input("PavedDrive: Paved driveway")
    input.loc[0, "SaleType"] = st.text_input("SaleType: Type of sale")
    input.loc[0, "SaleCondition"] = st.text_input("SaleCondition: Condition of sale")
st.text("")
st.markdown(
    """
    <style>
